...
import { ehrSection8 } from "@/constants/electronic-health-record";
import { ROUTES } from "@/constants/routes";

const AccentHeading = ({ headingLines }) => (
  <h2 className="text-center font-syne font-bold leading-[1.08] tracking-[-0.01em] text-[30px] text-white sm:text-[44px] md:text-[56px]">
...
const CTAButton = ({ text, href = ROUTES.REQUEST_DEMO }) => (
  <Link
    href={href}
    className="group relative mt-6 inline-flex h-12 items-center rounded-full bg-[#0A95E4] pl-6 pr-16 font-dm-sans text-sm font-medium text-white shadow-[0_10px_30px_rgba(10,149,228,0.35)] transition-all duration-300 hover:-translate-y-px hover:bg-[#0991de] sm:mt-8 sm:h-[60px] sm:pl-10 sm:pr-24 sm:text-[20px]"
  >
    {text}
...
const EHRSection8 = () => {
  const { headingLines, paragraph, ctaText } = ehrSection8;
